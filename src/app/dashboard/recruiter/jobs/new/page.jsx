...
import { Briefcase, Globe } from "@gravity-ui/icons";
import { createJob } from "@/lib/actions/jobs";
import { useRouter } from "next/navigation"; // Changed to useRouter to avoid redirect() breaking toast flows
...
export default function PostJobPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation constraint: check location ONLY if Onsite
        ...
        const payload = {
            ...data,
            isRemote,
            location: isRemote ? "Remote" : data.location, // Enforce fallbacks
            companyId: mockCompany.id,
            status: "active",
            isPubliclyVisible: true,
        };
        ...
        if (res?.insertedId) {
            ...
            e.target.reset();
            setIsRemote(false);
            router.push("/dashboard/recruiter/jobs"); // Safely route after tracking state updates
        }
    };

    // Style configs mapping
    const textInputClass = "w-full text-white bg-[#1c1c1e] border border-zinc-800 hover:bg-[#242426] focus:border-zinc-600 rounded-lg h-12 px-3 text-sm placeholder:text-zinc-600 outline-none transition-all";
    const textAreaClass = "w-full text-white bg-[#1c1c1e] border border-zinc-800 hover:bg-[#242426] focus:border-zinc-600 rounded-lg p-3 text-sm placeholder:text-zinc-600 outline-none transition-all";
    const selectBoxClass = "w-full";
    const triggerClasses = "w-full flex items-center justify-between bg-[#1c1c1e] border border-zinc-800 hover:bg-[#242426] h-12 rounded-lg px-3 text-white transition-all text-sm outline-none data-[focused=true]:border-zinc-600 data-[invalid=true]:border-danger";
            ...
            <div className="max-w-3xl mx-auto bg-[#121214] border border-zinc-900 rounded-xl p-8 shadow-2xl">
                ...
                <Form onSubmit={handleSubmit} className="space-y-8" validationErrors={errors} validationBehavior='aria'>
                    ...
                    <Fieldset className="space-y-6 w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <TextField isInvalid={!!errors.jobTitle} className="flex flex-col gap-1 w-full">
                                <Label className="text-zinc-400 font-medium text-sm">Job Title</Label>
                                <Input name="jobTitle" placeholder="e.g. Senior Frontend Engineer" className={textInputClass} />
                                {errors.jobTitle && <FieldError className="text-xs text-danger mt-1">{errors.jobTitle}</FieldError>}
                            </TextField>
                                ...
                                <div className="col-span-2 space-y-1">
                                    ...
                                    <div className="flex gap-2">
                                        <TextField isInvalid={!!errors.minSalary} className="w-full">
                                            <Input name="minSalary" placeholder="Min" type="number" className={textInputClass} />
                                        </TextField>
                                        <TextField isInvalid={!!errors.maxSalary} className="w-full">
                                            <Input name="maxSalary" placeholder="Max" type="number" className={textInputClass} />
                                        </TextField>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                            {/* Controlled Workplace Type Field block */}
                            <div className="space-y-2">
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-zinc-400 font-medium text-sm">Workplace Type</span>
                                ...
                                {/* Dynamic Rendering Wrapper */}
                                {!isRemote ? (
                                    <TextField isInvalid={!!errors.location} className="flex flex-col gap-1 w-full relative animate-in fade-in duration-200">
                                        <div className="relative flex items-center">
                                            <Globe size={16} className="absolute left-3 text-zinc-600 pointer-events-none z-10" />
                                            <Input
                                                name="location"
                                    ...
                                    <div className="h-12 flex items-center px-4 rounded-lg bg-zinc-900/40 border border-dashed border-zinc-800 text-xs text-zinc-500 italic">
                                        Position classified as worldwide distributed remote.
                                    </div>
                                )}
                            </div>

                            <TextField isInvalid={!!errors.deadline} className="flex flex-col gap-1 w-full">
                                <Label className="text-zinc-400 font-medium text-sm">Application Deadline</Label>
                    ...
                    <Fieldset className="space-y-6 w-full">
                        ...
                        <TextField isInvalid={!!errors.responsibilities} className="flex flex-col gap-1 w-full">
                            <Label className="text-zinc-400 font-medium text-sm">Responsibilities</Label>
                            <TextArea
                        ...
                        <TextField isInvalid={!!errors.requirements} className="flex flex-col gap-1 w-full">
                            <Label className="text-zinc-400 font-medium text-sm">Requirements</Label>
                            <TextArea
                                name="requirements"
                                placeholder="List required experience, skills, and certifications..."
